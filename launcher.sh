...
update_local_repo() {
	...
	if [ -f "$REPO_PATH${SEP}scripts${SEP}check_ps.sh" ]; then
		ln -f "$REPO_PATH${SEP}scripts${SEP}check_ps.sh" .
...
if [ -f "$REPO_PATH${SEP}scripts${SEP}get_ip_addr.sh" ]; then
...
if [ -f "$REPO_PATH${SEP}scripts${SEP}buildcreds.py" ]; then 
	python 3 scripts/buildcreds.py
	echo "[ $(ansi --green --bold flowreport_key updated!) ]"
else
	if [ ! -f "buildcreds.py" ]; then
		printf "\n[ $(ansi --red --bold ERROR: No 'buildcreds.py' file found!) ]\n"
		die
	fi
fi
...
if [ -f "watershed.py" ] && [ -f "sheet_manager.py" ]; then 
	...
	printf "\n[ $(ansi --red --bold ERROR: Missing essential 'watershed.py' and 'sheet_manager.py' files!) ]\n"
	die
fi
